...
FTP.prototype._setup = function(options, socket) {
  var self = this;
  ...
  if(this.options.socksproxy === undefined)
...
FTP.prototype.connect = function(options) {
  ...
  this.options.socksproxy = options.socksproxy || undefined;
  ...
  if(this.options.socksproxy === undefined){
	  ...
	  this._setup(options, socket);
  }else{
	var proxy = url.parse(this.options.socksproxy);
	var auth = proxy.auth;
	var user = auth.split(":")[0]
	var pass = auth.split(":")[1]
  var proxyport = proxy.port || 1080;
	...
	var options = {
	    ...
	    target: {
	        host: this.options.host, // When using bind, it's best to give an estimation of the ip that will be connecting to the newly opened tcp port on the proxy server.
	        port: this.options.port
	    }
	};
	
	Socks.createConnection(options, function(err, soc, info) {
	    if (err)
	        cb(makeError(999, 'SOCKS connect timed out'));
	    else {
	        socket = soc;
	      	socket.setTimeout(0);
		  	socket.setKeepAlive(true);
	        socket.resume();
	        self._setup(options, socket);
	    }
	});
...
FTP.prototype._pasvConnect = function(ip, port, cb) {
  ...
  if(this.options.socksproxy === undefined){
  	socket = new Socket();
  	self._pasvPostConnect(ip, port, cb, socket);
  }else{
	  	var proxy = url.parse(this.options.socksproxy);
		var auth = proxy.auth;
		var user = auth.split(":")[0]
		...
		Socks.createConnection(options, function(err, soc, info) {
		    if (err){
		        cb(makeError(999, 'PASV connect timed out'));
		    }else {
		        self._pasvPostConnect(ip, port, cb, soc);
   		        soc.resume();
		    }
		});
  }
...
FTP.prototype._pasvPostConnect = function(ip, port, cb, socket) {
  var self = this,
      sockerr,
      timedOut = false,
  ...
  if(this.options.socksproxy === undefined)
	 socket.once('connect', exeConnect);
  else
  	exeConnect();
